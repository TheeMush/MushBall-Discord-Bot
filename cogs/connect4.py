...
class Game:
    ...
    async def print_grid(self) -> None:
        ...
        title = (
            f':red_circle: Connect 4 :blue_circle: \n{self.player1.display_name}'
        ...
        formatted_grid = "\n".join([first_row] + rows)
        embed = discord.Embed(title=title, description=formatted_grid,color=discord.Colour.random())
    ...
    async def game_over(self, action: str, player1: discord.user, player2: discord.user,amount,ctx) -> None:
        """Announces to public chat."""
        if action == "win":
            if player1.id == self.bot.user.id:
                embed = discord.Embed(title=":red_circle: Game Over! :blue_circle:",description=f"{player1.mention} Wins!", color=discord.Colour.random())
                embed.set_footer(text="Imagine losing to a bot lol")
                await self.channel.send(embed=embed)
                await update_bank(ctx,player2.id,-1*amount,0)
                return

            embed = discord.Embed(title=":red_circle: Game Over! :blue_circle:",description=f"{player1.mention} Wins!\n\n**Winnings:** `${format (2*amount, ',d')}`", color=discord.Colour.random())
            await self.channel.send(embed=embed)
            await update_bank(ctx,player1.id,amount,0)
            await update_bank(ctx,player2.id,-1*amount,0)
        elif action == "draw":
            embed = discord.Embed(title=":red_circle: Game Over! :blue_circle:",description=f"It's A Draw!", color=discord.Colour.random())
            await self.channel.send(embed=embed)
        elif action == "quit":
            embed = discord.Embed(title=":red_circle: Game Over! :blue_circle:",description=f"{self.player1.mention} Quit", color=discord.Colour.random())
            await self.channel.send(embed=embed)
        await self.print_grid()

    async def start_game(self,amount,ctx) -> None:
        """Begins the game."""
        ...
        while True:
            ...
            if isinstance(self.player_active, AI):
                ...
                coords = await self.player_turn(amount,ctx)
    ...
    async def player_turn(self,amount,ctx) -> Coordinate:
        ...
        while True:
            try:
                ...
                if str(reaction.emoji) == CROSS_EMOJI:
                    await self.game_over("quit", self.player_active, self.player_inactive,amount,ctx)
                    return
...
class ConnectFour(commands.Cog):
    ...
    async def _play_game(
        ...
        try:
            ...
            await game.start_game(amount,ctx)
    ...
    async def connect_four(
        ...
        if amount>bal[0]:
            ...
            embed.set_author(name = "Connect 4",icon_url=ctx.author.avatar_url)
        ...
        embed = discord.Embed(title=":red_circle: Connect 4 :blue_circle:",description=f"{ctx.author.mention} started a Connect 4 game!\n\nReact with {Emojis.hand_raised} if you want to play\n\n**Bet Amount:** `{format (amount, ',d')}`", color=discord.Colour.random())
        embed.set_footer(text="React with X if you want to cancel")

        announcement = await ctx.send(embed=embed)
        ...
        bal2 = await update_bank(ctx,user.id,0,0)
        if amount>bal2[0]:
            embed = discord.Embed(title=":red_circle: Connect 4 :blue_circle:",description=f"{user.mention} doesn't have {format (amount, ',d')}!", color=red)
            await ctx.reply(embed=embed)
            return

        await self._play_game(ctx, user, board_size, str(emoji1), str(emoji2),amount)
    ...
    async def ai(
        ...
        try:
            ...
            if amount>bal[0]:
                embed = discord.Embed(description="You need atleast $500 to play", color=red)
                embed.set_author(name = "Connect 4",icon_url=ctx.author.avatar_url)
                await ctx.reply(embed=embed)
                return
            if amount<0:
            ...
            await self._play_game(ctx, None, board_size, str(emoji1), str(emoji2),amount)
        except:
            print(f"```{traceback.format_exc()}```")
